...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS users (
        ...
        vip_expiry TEXT,
        vip_uid TEXT,
        auto_like_active INTEGER DEFAULT 0,
        daily_like_limit INTEGER DEFAULT 0,
        daily_like_sent INTEGER DEFAULT 0,
        daily_like_date TEXT,
    ...
    try:
        ...
        if chat_id in ad_state and ad_state[chat_id].get("step") == "waiting_for_photo":
            if "photo" in update["message"]:
                ...
                send_message(chat_id, "📝 **سیو اکانت خود را بگویید**\n\nمثلاً: جیمیل، آیدی، شماره تماس یا هر چیز دیگر:\n(مثلاً: `example@gmail.com` یا `@user_id`)")
            else:
        ...
        if chat_id in ad_state and ad_state[chat_id].get("step") == "waiting_for_description":
            if text.strip():
                ...
                send_message(chat_id, "✅ **آگهی شما به ادمین ارسال شد!**\n\n📌 پس از تأیید ادمین، برای همه کاربران ارسال خواهد شد.\n⏳ لطفاً صبور باشید.")
                ad_state.pop(chat_id, None)
            else:
        ...
        if chat_id in admin_state and admin_state[chat_id].get("step") == "waiting_for_code_points":
            if text.isdigit():
                ...
                code = generate_code()
                # ذخیره در دیتابیس
                # برای سادگی، از یه جدول جداگانه استفاده نمیکنیم و توی یه کلید توی دیتابیس ذخیره میکنیم
                # فعلاً از همون روش JSON استفاده میکنیم برای کدها
                users_data = {}
                if os.path.exists("users.json"):
                    with open("users.json", "r") as f:
                        users_data = json.load(f)
                if "gift_codes" not in users_data:
                    users_data["gift_codes"] = {}
                users_data["gift_codes"][code] = {
                    "uses": uses,
                    "used": 0,
                    "points": points,
                    "users": []
        ...
        if text.startswith("/redeem"):
            ...
            if len(parts) == 2:
                ...
                if os.path.exists("users.json"):
                    with open("users.json", "r") as f:
                        users_data = json.load(f)
                if "gift_codes" in users_data and code in users_data["gift_codes"]:
                    gift_data = users_data["gift_codes"][code]
        ...
        if is_admin(chat_id) and chat_id in broadcast_data and broadcast_data[chat_id] == "waiting_for_message":
            ...
            for user in users:
                try:
                    if send_message(user["chat_id"], f"📢 **پیام همگانی:**\n\n{msg_text}"):
                        sent += 1
                    else:
                        failed += 1
                    time.sleep(0.1)
...
def handle_callback(chat_id, data):
    try:
        ...
        if data.startswith("ad_approve_"):
            ...
            for user in users:
                try:
                    if send_photo_by_id(user["chat_id"], photo, caption):
                        sent += 1
                    else:
                        failed += 1
                    time.sleep(0.1)
        ...
        if data == "auto_like":
            if is_vip_active(chat_id):
                update_user(chat_id, auto_like_active=1)
        ...
        if data.startswith("admin_"):
            ...
            if data == "admin_list":
                ...
                for user in users:
                    ...
                    if cnt > 20:
                        break
                    name = user.get("display_name") or user.get("username") or user["chat_id"]
                    vip_icon = ""
                    if user.get("vip_level") in ["silver", "gold", "crystal"]:
                        if user.get("vip_expiry"):
                            try:
                    ...
                    text += f"{cnt}. `{name}` {vip_icon} → 💎{user.get('score', 0)} {status}\n"
                if len(users) > 20:
            ...
            if data == "admin_stats":
                ...
                banned = sum(1 for user in users if user.get("is_banned", 0))
                text = f"📊 **آمار کل ربات:**\n\n"
        ...
        if data == "notification_settings":
            status = "فعال" if user.get("notifications", 1) else "غیرفعال"
            keyboard = [
        ...
        if data == "notif_on":
            update_user(chat_id, notifications=1)
            send_message(chat_id, "✅ **اعلان‌ها فعال شدند!**")
            send_message(chat_id, "💎 **حساب من**", my_score_menu())
            return

        if data == "notif_off":
            update_user(chat_id, notifications=0)
