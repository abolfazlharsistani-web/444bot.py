...
def send_message(chat_id, text, keyboard=None):
    ...
    for attempt in range(3):
        try:
            resp = requests.post(BASE + "sendMessage", json=data, timeout=30, proxies=proxies)
            ...
            print(f"⏳ Timeout در ارسال به {chat_id} (تلاش {attempt+1}/3)")
            time.sleep(3)
        except Exception as e:
...
def send_photo(chat_id, photo_url, caption=None, keyboard=None):
    ...
    for attempt in range(3):
        try:
            resp = requests.post(BASE + "sendPhoto", json=data, timeout=30, proxies=proxies)
            return resp.status_code == 200
        except requests.exceptions.Timeout:
            print(f"⏳ Timeout در ارسال عکس به {chat_id} (تلاش {attempt+1}/3)")
            time.sleep(3)
        except Exception as e:
...
def check_membership(chat_id):
    for attempt in range(3):
        try:
            ...
            resp = requests.get(BASE + "getChatMember", params=params, timeout=30, proxies=proxies)
            ...
            print(f"⏳ Timeout در check_membership برای {chat_id} (تلاش {attempt+1}/3)")
            time.sleep(3)
        except Exception as e:
...
def like_amount_menu():
    return [
        [{"text": "💯 100 لایک (۳ جم)", "callback_data": "like_100"}],
        [{"text": "💯 200 لایک (۶ جم)", "callback_data": "like_200"}],
        [{"text": "💯 300 لایک (۹ جم)", "callback_data": "like_300"}],
        [{"text": "💯 400 لایک (۱۲ جم)", "callback_data": "like_400"}],
        [{"text": "💯 500 لایک (۱۵ جم)", "callback_data": "like_500"}],
...
def handle_callback(chat_id, data):
    try:
        ...
        if data in ["like_100", "like_200", "like_300", "like_400", "like_500"]:
            ...
            price = (amount // 100) * 3  # قیمت جدید: ۳ جم به ازای هر ۱۰۰ لایک
